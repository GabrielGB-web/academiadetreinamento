...
import { useCourseWithProgress } from '@/hooks/useCoursesWithProgress';

export default function CourseDetail() {
  const { courseId } = useParams();
  const [openModules, setOpenModules] = useState<string[]>([]);
  
  const { data: course, isLoading } = useCourseWithProgress(courseId);
    ...
    return (
      <MainLayout>
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mt-2">Carregando curso...</p>
        </div>
      </MainLayout>
    );
  }
  ...
  const firstIncompleteLessonId = getFirstIncompleteLesson();
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="p-6 rounded-xl gradient-card shadow-card border border-border/50 h-fit animate-scale-in">
          ...
          {firstIncompleteLessonId ? (
            <Link to={`/aula/${firstIncompleteLessonId}`}>
      ...
      <div className="space-y-4">
        ...
        {course.modules.length === 0 ? (
          ...
          course.modules.map((module, moduleIndex) => {
            ...
            const moduleCompleted = module.lessons.length > 0 && module.lessons.every((l) => l.completed);
